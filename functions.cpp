...
void initSystem(Facebook& system) // hard-coded entities:
{
	...
	Status s4("I like to eat Happy-Hippo's");
	system.getUser(1)->addStatusForUser(s3); system.getUser(0)->addStatusForUser(s4);
	Status s5("Hi, my name is Liav and I live in Oranit");
	Status s6("I like to eat a lot of scarambled eggs");
	...
	Status s10("The sale has opened for discounted tours in the Carmel Market for the residents of the city");
	...
	Status s12("Agricultural land in Evan Yehuda, please respond if you are interested");
...
void addStatusToFanPage(Facebook& system)
{
	...
	Status newStatus(newTextStatus); // Create a new status
